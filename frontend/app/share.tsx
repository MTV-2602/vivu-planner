...
import { View, Text, ScrollView, ActivityIndicator, Platform, Pressable, Linking } from 'react-native';
...
export default function PublicShareTripPage() {
  ...
  // Also parse from window.location.search if on web and params.id is missing
  const tripId = params.id || (typeof window !== 'undefined' ? new URLSearchParams(window.location.search).get('id') : '') || '';
  ...
  useEffect(() => {
    if (!tripId) return;
    apiClient.get(`/trips/${tripId}/public`)
  ...
  if (loading) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#FBF5EA', minHeight: 400 }}>
  ...
  if (error || !trip) {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#FBF5EA', padding: 32, minHeight: 400 }}>
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: '#FBF5EA' }}>
      {/* Hero Banner */}
      <View style={{ backgroundColor: BRAND_COLORS.primary, padding: 32, paddingTop: 48 }}>
        ...
        <View style={{ flexDirection: 'row', gap: 12, flexWrap: 'wrap' }}>
          {[
            ...
            <View key={stat.label} style={{ flex: 1, minWidth: 120, backgroundColor: 'rgba(255,255,255,0.12)', borderRadius: 12, padding: 12, alignItems: 'center' }}>
      ...
      <View style={{ padding: 20, gap: 20 }}>
        ...
        <View style={{ backgroundColor: '#fff', borderRadius: 20, padding: 20, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10 }}>
          <Text style={{ fontSize: 17, fontWeight: '800', color: '#1B3A2D', marginBottom: 16 }}>🗺️ Bản đồ lịch trình (Google Maps Tiles)</Text>
          <InteractiveMap
            items={allMapItems}
                    ...
                    <Pressable
                      onPress={() => {
                        const mapsUrl = item.location_lat && item.location_lng
                          ...
                          : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.title + ' ' + trip.destination_city)}`;
                        if (Platform.OS === 'web') window.open(mapsUrl, '_blank');
                        else Linking.openURL(mapsUrl);
                      }}
                      style={{ paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6, backgroundColor: '#e8f5f0' }}
                    >
                      <Text style={{ color: BRAND_COLORS.primary, fontSize: 11, fontWeight: '700' }}>📍 Google Maps ↗</Text>
                    </Pressable>
                  </View>
