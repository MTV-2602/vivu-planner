...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<'users'|'packages'|'prices'|'revenue'|'trips'|'keys'|'partners'>('revenue');
  ...
  useEffect(() => {
    if (plansData?.plans) {
      ...
      if (p.vip?.amount != null) setVipPrice(String(p.vip.amount));
    }
  }, [plansData]);
  ...
  const updatePackageMutation = useMutation({
    mutationFn: async ({ userId, is_premium, plan, custom_quota }: { userId: string; is_premium: boolean; plan?: string; custom_quota: number }) => {
      await apiClient.put(`/admin/users/${userId}/package`, { is_premium, plan, custom_quota });
    },
    onSuccess: () => {
    ...
    <View className="flex-1 bg-brand-bg">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 24, gap: 24 }}>
        ...
        <View className="flex-row border-b border-brand-line/40 gap-0 flex-wrap">
          {(['users', 'packages', 'prices', 'revenue', 'trips', 'keys', 'partners'] as const).map(tab => {
            const labels = {
              users: 'Người dùng',
              packages: 'Gói thành viên 👑',
              prices: 'Cấu hình Giá ⚙️',
