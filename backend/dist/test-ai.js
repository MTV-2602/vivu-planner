...
async function runTests() {
    ...
    const coords = (0, placesService_1.getCityCoordinates)('Đà Nẵng');
    ...
    const weather = await (0, weatherService_1.getWeatherForecast)(coords.lat, coords.lng, '2026-06-20', '2026-06-22');
    ...
    const lodgings = await (0, placesService_1.searchPlaces)('khách sạn', 'accommodation', coords.lat, coords.lng);
    ...
    const dining = await (0, placesService_1.searchPlaces)('nhà hàng ngon', 'dining', coords.lat, coords.lng);
    const attractions = await (0, placesService_1.searchPlaces)('điểm tham quan', 'attraction', coords.lat, coords.lng);
    const rentals = await (0, placesService_1.searchPlaces)('thuê xe', 'rental', coords.lat, coords.lng);
    ...
    const itinerary = await (0, geminiService_1.generateItinerary)(tripData, weather, candidatePlaces);
    ...
    const { itinerary: adapted, diff } = await (0, geminiService_1.adaptItinerary)(tripData, itinerary, 'weather_change', 'Mưa bão lụt lớn toàn thành phố Đà Nẵng, gió to sạt lở Sơn Trà', weather, candidatePlaces);
    ...
    const hasAdjusted = adapted.days.some(day => day.items.some(item => item.title.includes('Thay đổi') || item.title.includes('Nghỉ ngơi') || item.title.includes('Tiết kiệm') || item.title.includes('trời')));
