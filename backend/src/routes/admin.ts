...
import { autoCancelExpiredOrders, loadPlansFromDb } from './payment';
...
router.get('/user-packages', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const userPackages = (users || []).map((u: any) => {
      ...
      const tripsQuota = prof?.custom_quota || (isPremium ? 9999 : 3);
  ...
  try {
    ...
    if (is_premium) {
      ...
      await supabaseAdmin.from('payment_orders').insert({
        ...
        method: 'admin',
        plan,
        amount: plan === 'starter' ? 29000 : 49000,
        status: 'completed',
    ...
    const { data, error } = await supabaseAdmin
      ...
      .upsert({
        ...
        premium_until,
        custom_quota: custom_quota != null ? Number(custom_quota) : (is_premium ? 9999 : 3),
      })
      .select()
      .single();
...
router.post('/plans', async (req: AuthenticatedRequest, res: Response) => {
  const { starter, premium, vip } = req.body;
  try {
    if (starter) {
