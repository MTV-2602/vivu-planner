...
import { EXTERNAL_APIS, TIMEOUTS } from '../../constants';
...
export async function getWeatherForecast(
  ...
  try {
    const url = `${EXTERNAL_APIS.OPEN_METEO_BASE_URL}?latitude=${lat}&longitude=${lng}&daily=weather_code,temperature_2m_max,temperature_2m_min,precipitation_probability_max&timezone=auto&start_date=${startDate}&end_date=${endDate}`;
    const response = await axios.get(url, { timeout: TIMEOUTS.WEATHER_API_MS });
