...
router.get('/:id', requireAuth, async (req: any, res: Response) => {
  ...
  try {
    ...
    // Count how many trips this user created before or at the same time as this trip
    const { count: priorTripsCount } = await supabaseAdmin
      .from('trips')
      .select('id', { count: 'exact', head: true })
      .eq('user_id', trip.user_id)
      .lte('created_at', trip.created_at);

    const isFreeTier = (priorTripsCount ?? 0) <= 3;
    ...
    itinerary.days.forEach(day => {
      ...
      day.items.forEach(item => {
        ...
        if (item.google_place_id) {
          ...
          if (matched) {
            if (!item.lat) itemLat = matched.lat;
            if (!item.lng) itemLng = matched.lng;
