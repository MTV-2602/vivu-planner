...
import { GEO_CONFIG, BUDGET_PRICE_TIERS, PARTNER_MATCH_WEIGHTS } from '../../constants';
...
function getHaversineDistance(lat1: number, lon1: number, lat2: number, lon2: number): number {
  const R = GEO_CONFIG.EARTH_RADIUS_KM; // Radius of the earth in km
...
function getBudgetPriceLevel(budgetTotal: number, startDate: string, endDate: string, travelerCount: number): number {
  try {
    ...
    if (dailyBudget < BUDGET_PRICE_TIERS.BUDGET_MAX) return 1; // budget ($)
    if (dailyBudget < BUDGET_PRICE_TIERS.MID_MAX) return 2; // mid ($)
    if (dailyBudget < BUDGET_PRICE_TIERS.UPSCALE_MAX) return 3; // upscale ($$)
    return 4; // luxury ($$$$)
  } catch {
  ...
  try {
    ...
    const scoredPartners = partners
      ...
      .map((partner: Partner) => {
        ...
        if (priceDiff === 0) preferenceScore += PARTNER_MATCH_WEIGHTS.EXACT_PRICE_MATCH;
        else if (priceDiff === 1) preferenceScore += PARTNER_MATCH_WEIGHTS.CLOSE_PRICE_MATCH;
        ...
        if (tripPreferences.food === true && (partner.category === 'restaurant' || partner.category === 'cafe' || partner.category === 'dining')) {
          preferenceScore += PARTNER_MATCH_WEIGHTS.CUISINE_MATCH;
        }
        if (tripPreferences.relax === true && ['hotel', 'homestay', 'resort', 'accommodation'].includes(partner.category)) {
          preferenceScore += PARTNER_MATCH_WEIGHTS.AMENITY_MATCH;
        }
        if (tripPreferences.nature === true) {
          ...
          if (hasNatureTag || partner.category === 'resort') {
            preferenceScore += PARTNER_MATCH_WEIGHTS.INTEREST_MATCH;
          }
        }
